...
function syncQueryParams(): URLSearchParams {
  ...
  if (match) params.set("article", match[1]);
  return params;
}
...
async function load(): Promise<void> {
  ...
  if (body.items.length) {
    root.innerHTML = body.items.map((a: { id: number; title: string; summary: string; category: string; type: string; updatedAt: string }) => `<article><small>${esc(a.type)} · ${esc(a.category)}</small><h2><a href="/knowledge-base/${a.id}">${esc(a.title)}</a></h2><p>${esc(a.summary)}</p></article>`).join("");
  } else {
...
async function detail(): Promise<void> {
  const match = location.pathname.match(/^\/knowledge-base\/(\d+)$/);
  if (!match) return void load();
