...
async function loadDetail(ticket: string): Promise<void> {
  ...
  $("#detail").innerHTML = `<article style="padding: 16px; border: 1px solid #ffffff12; border-radius: 11px; background: #172239; margin-top: 18px;"><h2 style="margin: 0 0 10px; font-family: 'Space Grotesk', sans-serif; font-size: 1.4rem; letter-spacing: -.03em;">${esc(ticket)} — ${esc(ticketData.subject)}</h2><p style="color: #9ba9bf; font-size: .9rem; margin: 0 0 18px;">Status: ${esc(ticketData.status)} · Category: ${esc(ticketData.category)} · Created: ${esc(ticketData.createdAt)}${ticketData.dueDate ? ` · Due: ${esc(ticketData.dueDate)}` : ""}${ticketData.isEscalated ? " · Escalated" : ""}</p><div style="padding: 16px; border: 1px solid #ffffff12; border-radius: 12px; background: #101827cc;"><h3 style="margin: 0 0 12px; font-size: 1rem; color: #dbe5f2;">History</h3>${history.map((h: { label: string; createdAt: string }) => `<div style="padding: 10px 0; border-top: 1px solid #ffffff12;"><div style="color: #c5d0df;">${esc(h.label)}</div><small style="color: #718198;">${esc(h.createdAt)}</small></div>`).join("")}</div><form id="feedback" style="margin-top: 18px; display: grid; gap: 10px; max-width: 520px;"><label style="margin: 18px 0 8px; color: #dbe5f2; font-size: .86rem; font-weight: 600;">Rating (1–5) <input name="rating" type="number" min="1" max="5" required style="width: 100%; padding: 14px 15px; color: #f7f8f2; border: 1px solid #536178; border-radius: 10px; outline: none; background: #1b263b; font: inherit;" /></label><textarea name="message" maxlength="2000" placeholder="Your feedback" required style="width: 100%; padding: 14px 15px; color: #f7f8f2; border: 1px solid #536178; border-radius: 10px; outline: none; background: #1b263b; font: inherit; min-height: 120px; resize: vertical;"></textarea><button type="submit" style="width: 100%; margin-top: 24px; padding: 14px 15px; color: #202337; border: 0; border-radius: 10px; background: #f4d35e; cursor: pointer; font: inherit; font-weight: 700;">Submit feedback</button><p id="feedback" role="alert" style="min-height: 22px; margin: 14px 0 0; color: #ff9b8a; font-size: .86rem;"></p></form></article>`;
  const feedbackForm = $<HTMLFormElement>("#feedback");
  feedbackForm.onsubmit = async (event: Event) => {
    ...
    const output = $<HTMLElement>("#feedback");
...
$<HTMLFormElement>("#customer-login").addEventListener("submit", async (event: Event) => {
  event.preventDefault();
  const form = event.currentTarget as HTMLFormElement;
  ...
  if (!response.ok) {
    $<HTMLElement>("#login-error")!.textContent = (await response.json()).error ?? "Unable to sign in.";
    return;
  }
...
$<HTMLButtonElement>("#list-tickets")?.addEventListener("click", () => {
...
window.addEventListener("popstate", async () => {
  const params = new URLSearchParams(location.search);
  const ticket = params.get("ticket");
  if (ticket) await loadDetail(ticket.toUpperCase());
  else {
    $("#detail").innerHTML = "";
    void loadTickets();
  }
