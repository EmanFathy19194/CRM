...
export async function renderTicketDetails(id) {
    ...
    const [ticketResponse, historyResponse, communicationsResponse, commentsResponse] = await Promise.all([
        fetch(`/api/tickets/${id}`, { credentials: "same-origin" }), fetch(`/api/tickets/${id}/history`, { credentials: "same-origin" }), fetch(`/api/tickets/${id}/communications`, { credentials: "same-origin" }), fetch(`/api/tickets/${id}/comments`, { credentials: "same-origin" })
    ]);
    ...
    categorySelect.innerHTML = categoryValues.map((category) => `<option value="${value(category)}"${category === ticket.category ? " selected" : ""}>${value(category)}</option>`).join("");
    categoryInput.replaceWith(categorySelect);
    const deadline = (date, stopped) => !date ? "Not configured" : stopped ? `Stopped ${value(new Date(date).toLocaleString())}` : `${new Date(date).getTime() < Date.now() ? "Overdue" : "Due"} ${value(new Date(date).toLocaleString())}`;
    const slaSection = document.createElement("section");
    slaSection.className = "ticket-history sla-state";
    slaSection.innerHTML = `<span class="eyebrow">SLA</span>${ticket.slaRuleId ? `<article><strong>Rule #${ticket.slaRuleId}</strong><span>Response target: ${ticket.responseTargetMinutes} minutes — ${deadline(ticket.responseDueAt, Boolean(ticket.responseRespondedAt))}</span><span>Resolution target: ${ticket.resolutionTargetMinutes} minutes — ${deadline(ticket.resolutionDueAt, ["resolved", "closed"].includes(ticket.status))}</span>${ticket.responseRespondedAt ? `<small>First response sent ${value(new Date(ticket.responseRespondedAt).toLocaleString())}</small>` : `<button id="mark-responded" type="button">Mark first response sent</button>`}</article>` : "<p>No SLA rule matched this ticket.</p>"}`;
    document.querySelector("#ticket-edit-form").insertAdjacentElement("afterend", slaSection);
    document.querySelector("#mark-responded")?.addEventListener("click", async (event) => { const button = event.currentTarget; button.disabled = true; const response = await fetch(`/api/tickets/${id}/responded`, { method: "POST", credentials: "same-origin" }); if (response.ok)
    ...
    document.querySelector("#quick-reply").addEventListener("change", event => { const reply = event.currentTarget.value; if (reply)
        document.querySelector("#comment-body").value = reply; });
    document.querySelector("#comment-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget, button = form.querySelector("button"), error = form.querySelector("#comment-error"); button.disabled = true; error.textContent = ""; const response = await fetch(`/api/tickets/${id}/comments`, { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(form).entries())) }); if (response.ok) {
        await renderTicketDetails(id);
        return;
    } const body = await response.json().catch(() => ({})); error.textContent = Object.values(body.errors ?? {}).join(" ") || body.error || "Unable to add internal comment."; button.disabled = false; });
