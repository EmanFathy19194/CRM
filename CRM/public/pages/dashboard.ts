...
type Notification = { id:number; ticketId:number; message:string; createdAt:string };
type Dashboard = { assignedTickets:Ticket[]; counts:{assigned:number;open:number;pending:number;urgent:number}; reminders:Reminder[]; notifications:Notification[]; recentActivity:Activity[] };
...
const api=(url:string,init?:RequestInit)=>fetch(url,{credentials:"same-origin",...init,headers:{"Content-Type":"application/json",...(init?.headers??{})}});
...
export async function renderDashboard() {
  ...
  const ticketLink=(ticket:Ticket)=>`<article><a href="/tickets/${ticket.id}"><strong>${value(ticket.ticketNumber)} · ${value(ticket.subject)}</strong></a><small>${value(ticket.customerName)} · ${value(ticket.customerEmail)}</small></article>`;
  ...
  content.querySelector<HTMLFormElement>("#reminder-form")!.addEventListener("submit",async event=>{event.preventDefault();const form=event.currentTarget as HTMLFormElement,button=form.querySelector<HTMLButtonElement>("button")!;button.disabled=true;if((await api("/api/reminders",{method:"POST",body:JSON.stringify(Object.fromEntries(new FormData(form).entries()))})).ok)refresh();else button.disabled=false;});
