...
let statusFilter = "";
let pageNumber = 1;
let latestSearchTimer: ReturnType<typeof setTimeout> | undefined;
...
export function scheduleCustomerSearch(
  ...
  if (latestSearchTimer !== undefined) clearTimer(latestSearchTimer);
...
export async function renderCustomers(page = pageNumber, search = searchTerm, status = statusFilter) {
  ...
  document.querySelector<HTMLInputElement>("#customer-search")!.addEventListener("input", (event) => { const value = (event.target as HTMLInputElement).value; scheduleCustomerSearch((next) => void renderCustomers(1, next, statusFilter), value); });
