...
export async function renderAdminUsers(): Promise<void> {
  ...
  const dialog = content.querySelector<HTMLDialogElement>("#user-dialog")!;
  const form = content.querySelector<HTMLFormElement>("#user-dialog-form")!;
  const title = content.querySelector<HTMLElement>("#dialog-title")!;
  const confirmBtn = content.querySelector<HTMLButtonElement>("#dialog-confirm")!;
  const messageEl = content.querySelector<HTMLElement>("#dialog-message")!;
  const errorEl = content.querySelector<HTMLElement>("#dialog-error")!;
  let editingId: number | null = null;

  async function load(): Promise<void> {
    const response = await fetch("/api/admin/users", { credentials: "same-origin" });
    if (!response.ok) { content.querySelector<HTMLElement>("#user-list")!.innerHTML = "<p>Unable to load users.</p>"; return; }
    const data = await response.json();
    const list = content.querySelector<HTMLElement>("#user-list")!;
    if (!data.items.length) { list.innerHTML = "<p>No users found.</p>"; return; }
    list.innerHTML = data.items.map((u: { id: number; name: string; email: string; role: string; isActive: boolean; createdAt: string }) => `<article class="user-row"><div><strong>${escapeHtml(u.name)}</strong><span>${escapeHtml(u.email)}</span><small>${escapeHtml(u.role)} · ${u.isActive ? "Active" : "Inactive"} · ${new Date(u.createdAt).toLocaleDateString()}</small></div><div class="user-actions"><button data-view="${u.id}">View</button><button data-edit="${u.id}">Edit</button><button data-delete="${u.id}" class="danger">Delete</button></div></article>`).join("");
    list.querySelectorAll<HTMLButtonElement>("[data-view]").forEach((btn) => btn.addEventListener("click", () => void view(Number(btn.dataset.view))));
    list.querySelectorAll<HTMLButtonElement>("[data-edit]").forEach((btn) => btn.addEventListener("click", () => void edit(Number(btn.dataset.edit))));
    list.querySelectorAll<HTMLButtonElement>("[data-delete]").forEach((btn) => btn.addEventListener("click", () => void remove(Number(btn.dataset.delete))));
  }
  ...
  async function view(id: number): Promise<void> {
    ...
    if (result === "confirmed") dialog.close();
  }
  ...
  async function edit(id: number | null = null): Promise<void> {
    ...
    if (id !== null) {
      const response = await fetch(`/api/admin/users/${id}`, { credentials: "same-origin" });
      if (!response.ok) { errorEl.textContent = "User not found."; return; }
      const user = await response.json();
      title.textContent = "Edit user";
      confirmBtn.textContent = "Save changes";
      messageEl.textContent = `Editing ${user.name}`;
      form.querySelector<HTMLInputElement>("input[name=name]")!.value = user.name;
      form.querySelector<HTMLInputElement>("input[name=email]")!.value = user.email;
      form.querySelector<HTMLSelectElement>("select[name=role]")!.value = user.role;
      form.querySelector<HTMLInputElement>("input[name=password]")!.value = "";
      form.querySelector<HTMLInputElement>("input[name=password]")!.placeholder = "Leave blank to keep current password";
    } else {
      editingId = null;
      ...
      form.reset();
      form.querySelector<HTMLInputElement>("input[name=password]")!.placeholder = "";
    }
    dialog.showModal();
  }
  ...
  form.addEventListener("submit", async (event: Event) => {
    ...
    if (response.ok) { dialog.close(); void load(); }
    else { const body = await response.json(); errorEl.textContent = body.errors ? Object.values(body.errors).join(" ") : body.error ?? "Unable to save user."; }
  });

  dialog.addEventListener("close", () => { editingId = null; errorEl.textContent = ""; });
  content.querySelector<HTMLButtonElement>("#create-user")!.addEventListener("click", () => void edit(null));
  await load();
}
