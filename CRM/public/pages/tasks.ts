...
type Task = { id:number; title:string; details:string|null; dueAt:string|null; isCompleted:boolean };
...
export async function renderTasks() {
  ...
  if (!response.ok) { content.innerHTML = `<div class="customer-error"><strong>We could not load your tasks.</strong></div>`; return; }
  ...
  const form = content.querySelector<HTMLFormElement>("#task-form")!;
  form.addEventListener("submit", async (event) => {
    event.preventDefault();
    const button = form.querySelector<HTMLButtonElement>("button")!, error = form.querySelector<HTMLElement>("#task-error")!;
    button.disabled = true; error.textContent = "";
    const result = await api("/api/tasks", { method:"POST", body:JSON.stringify(Object.fromEntries(new FormData(form).entries())) });
    if (result.ok) refresh();
    else { const body = await result.json().catch(() => ({})) as { errors?: Record<string,string>; error?: string }; error.textContent = Object.values(body.errors ?? {}).join(" ") || body.error || "Unable to add task."; button.disabled = false; }
  });
  const mutate = (selector:string, url:(button:HTMLButtonElement)=>string, method:string) => content.querySelectorAll<HTMLButtonElement>(selector).forEach(button => button.addEventListener("click", async () => { button.disabled = true; if ((await api(url(button), { method })).ok) refresh(); else button.disabled = false; }));
  ...
  content.querySelectorAll<HTMLButtonElement>("[data-task-edit]").forEach(button => button.addEventListener("click", async () => {
    const task = tasks.find(item => item.id === Number(button.dataset.taskEdit)); if (!task) return;
    const title = await showDialog({ title: "Edit task", message: "Update the task title.", input: task.title, confirmLabel: "Save task" }); if (title === null) return;
    button.disabled = true;
    if ((await api(`/api/tasks/${task.id}`, { method:"PATCH", body:JSON.stringify({ title, details:task.details ?? "", dueAt:task.dueAt ?? "" }) })).ok) refresh(); else button.disabled = false;
  }));
}
