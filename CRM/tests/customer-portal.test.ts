...
describe("customer portal integration", () => {
  ...
  async function createCustomerAndTicket(email: string, firstName: string, lastName: string): Promise<{ app: ReturnType<typeof createApp>; agent: ReturnType<typeof request.agent>; customerId: number; ticketNumber: string }> {
  ...
  it("lists and details customer-scoped tickets with safe projections", async () => {
    ...
    expect(history.body.every((h: { changedBy?: string }) => !h.changedBy)).toBe(true);
  });
