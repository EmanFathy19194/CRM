...
describe("public web requests", () => {
  ...
  it("creates a ticket plus web_form communication and returns only the ticket number and portal url", async () => {
    ...
    expect(communications).toHaveLength(1); expect(communications[0].channel).toBe("web_form");
    const history = (await agent.get("/api/tickets/1/history")).body;
    expect(history.filter((entry: { changedBy: string }) => entry.changedBy === "web-form").length).toBeGreaterThanOrEqual(1);
  });
