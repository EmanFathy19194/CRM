...
describe("communications", () => {
  ...
  async function signedInAgent() {
    ...
    return { agent, customerId: customer.body.id as number };
  }
  const message = (customerId: number, overrides: Record<string, unknown> = {}) => ({ customerId, channel: "email", message: "Customer called about billing.", ...overrides });
  ...
  it("lists and toggles the five configured channels", async () => {
    ...
    expect(channels.every((channel: { isEnabled: boolean }) => channel.isEnabled)).toBe(true);
    const disabled = await agent.patch("/api/communication-channels/sms").send({ enabled: false });
    expect(disabled.status).toBe(200); expect(disabled.body.isEnabled).toBe(false);
  ...
  it("records incoming messages for every configured channel with history events", async () => {
    ...
    for (let id = 1; id <= 4; id++) {
      ...
      expect(history.filter((entry: { action: string }) => entry.action === "communication_received")).toHaveLength(1);
    }
  });
  ...
  it("links to an existing same-customer ticket without changing ticket fields", async () => {
    ...
    expect(created.status).toBe(201); expect(created.body.ticketId).toBe(ticket.id);
    const updatedTicket = (await agent.get(`/api/tickets/${ticket.id}`)).body;
    expect(updatedTicket.status).toBe("open"); expect(updatedTicket.priority).toBe("high");
  ...
  it("creates a ticket when no ticket id is supplied", async () => {
    ...
    expect((await agent.get(`/api/tickets/${created.body.ticketId}/history`)).body.map((entry: { action: string }) => entry.action)).toEqual(expect.arrayContaining(["created", "communication_received"]));
  });
