...
export function createDatabase(location = process.env.CRM_DATABASE_PATH ?? join(process.cwd(), "data", "crm.sqlite")) {
  ...
  CREATE TABLE IF NOT EXISTS customer_interactions (
   ...
   CREATE INDEX IF NOT EXISTS customer_interactions_customer_idx ON customer_interactions (customer_id, created_at DESC, id DESC);`);
    try {
        database.exec(`ALTER TABLE customers ADD COLUMN password_hash TEXT;`);
    }
    catch { /* Existing databases already have the column. */ }
  ...
  CREATE INDEX IF NOT EXISTS ticket_history_ticket_idx ON ticket_history (ticket_id, created_at DESC, id DESC);`);
    try {
        database.exec(`ALTER TABLE support_tickets ADD COLUMN sla_rule_id INTEGER;`);
    }
    catch { /* Existing databases already have the column. */ }
    try {
        database.exec(`ALTER TABLE support_tickets ADD COLUMN response_target_minutes INTEGER;`);
    }
    catch { /* Existing databases already have the column. */ }
    try {
        database.exec(`ALTER TABLE support_tickets ADD COLUMN response_due_at TEXT;`);
    }
    catch { /* Existing databases already have the column. */ }
    try {
        database.exec(`ALTER TABLE support_tickets ADD COLUMN response_responded_at TEXT;`);
    }
    catch { /* Existing databases already have the column. */ }
    try {
        database.exec(`ALTER TABLE support_tickets ADD COLUMN resolution_target_minutes INTEGER;`);
    }
    catch { /* Existing databases already have the column. */ }
    try {
        database.exec(`ALTER TABLE support_tickets ADD COLUMN resolution_due_at TEXT;`);
    }
    catch { /* Existing databases already have the column. */ }
