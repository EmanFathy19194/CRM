...
function isManagerOrAdmin(request, auth) { return isAdmin(request, auth); }
export function createApp(auth, customerRepository = new CustomerRepository(createDatabase()), ticketRepository = new TicketRepository(customerRepository.getDatabase())) {
    ...
    const report = (name) => (request, response) => { const { value, errors } = validateReportRange(request.query); if (Object.keys(errors).length)
        return response.status(400).json({ errors }); return response.json(reportsRepository[name](value)); };
    app.get("/api/reports/tickets", report("tickets"));
    app.get("/api/reports/sla", report("sla"));
    app.get("/api/reports/agents", report("agents"));
    app.get("/api/reports/satisfaction", report("satisfaction"));
    app.get("/api/reports/management", report("management"));
    ...
    app.post("/api/admin/users", async (request, response) => { const { value, errors } = validateStaffUser(request.body ?? {}); if (Object.keys(errors).length)
        return response.status(400).json({ errors }); try {
        return response.status(201).json(await administrationRepository.create(value, actor(request)));
    }
    catch {
        return response.status(400).json({ error: "Unable to create user." });
    } });
    app.get("/api/admin/users/:id", (request, response) => { const id = parsePositiveInteger(request.params.id); if (!id)
        return response.status(400).json({ error: "Invalid user id." }); const item = administrationRepository.get(id); return item ? response.json(item) : response.status(404).json({ error: "User not found." }); });
    app.patch("/api/admin/users/:id", async (request, response) => { const id = parsePositiveInteger(request.params.id), result = validateStaffUser(request.body ?? {}, true); if (!id)
        return response.status(400).json({ error: "Invalid user id." }); if (Object.keys(result.errors).length)
        return response.status(400).json({ errors: result.errors }); try {
        const item = await administrationRepository.update(id, result.value, actor(request));
        return item ? response.json(item) : response.status(404).json({ error: "User not found." });
    }
    catch {
        return response.status(400).json({ error: "Unable to update user." });
    } });
    app.post("/api/admin/users/:id/deactivate", (request, response) => { const id = parsePositiveInteger(request.params.id); if (!id)
        return response.status(400).json({ error: "Invalid user id." }); const result = administrationRepository.deactivate(id, actor(request)); return result === "ok" ? response.status(200).json({ deactivated: true }) : result === "missing" ? response.status(404).json({ error: "User not found." }) : response.status(409).json({ error: "This user cannot be deactivated." }); });
    app.delete("/api/admin/users/:id", (request, response) => { const id = parsePositiveInteger(request.params.id); if (!id)
        return response.status(400).json({ error: "Invalid user id." }); const result = administrationRepository.delete(id, actor(request)); return result ? response.status(204).send() : response.status(404).json({ error: "User not found." }); });
    ...
    app.patch("/api/admin/settings", (request, response) => { const { value, errors } = validateSettings(request.body ?? {}); if (Object.keys(errors).length)
        return response.status(400).json({ errors }); try {
        return response.json(administrationRepository.updateSettings(value, actor(request)));
    }
    catch {
        return response.status(500).json({ error: "Unable to update settings." });
    } });
    ...
    app.get(protectedPagePaths, (request, response) => {
        ...
        if (request.path === "/reports" && !isManagerOrAdmin(request, auth))
            return response.status(403).send("Manager access required.");
        if (!isAdmin(request, auth) && !["/dashboard", "/tickets", "/tasks", "/activities", "/reports"].includes(request.path))
