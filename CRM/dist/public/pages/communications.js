...
export async function renderCommunications() {
    ...
    const renderList = () => {
        const customerId = Number((document.querySelector("#filter-customer")).value);
      ...
      <article class="communication-row">
        ...
        list.querySelectorAll("a").forEach((link) => link.addEventListener("click", (event) => { event.preventDefault(); history.pushState({}, "", link.getAttribute("href")); void import("./shared.js").then((shared) => shared.route()); }));
    };
    renderList();
    document.querySelector("#filter-customer").addEventListener("change", renderList);
    document.querySelector("#filter-channel").addEventListener("change", renderList);
    ...
    document.querySelector("#communication-form").addEventListener("submit", async (event) => {
        ...
        const form = event.currentTarget, button = form.querySelector("button[type=submit]"), error = form.querySelector("#communication-error");
        button.disabled = true;
        ...
        if (!response.ok) {
            error.textContent = Object.values(body.errors ?? {}).join(" ") || body.error || "Unable to record communication.";
            button.disabled = false;
            return;
        }
        form.reset();
        error.textContent = "Communication recorded.";
        button.disabled = false;
        const refreshed = await fetch("/api/communications", { credentials: "same-origin" });
        if (refreshed.ok) {
            communications = await refreshed.json();
            renderList();
        }
    });
}
