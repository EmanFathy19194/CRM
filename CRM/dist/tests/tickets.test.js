...
describe("support tickets", () => {
    ...
    const ticketInput = (customerId) => ({ customerId, subject: "Cannot sign in", description: "The customer cannot access the dashboard.", category: "Access", priority: "high", assignedAgent: "agent@example.com", status: "new", dueDate: "2026-09-01" });
    it("creates, lists, updates, escalates, and records immutable history", async () => {
        ...
        expect(created.body.ticketNumber).toBe("TKT-000001");
        expect(created.body.customerName).toBe("Ada Lovelace");
        ...
        expect(history.body.map((entry) => entry.action)).toEqual(expect.arrayContaining(["created", "status_changed", "priority_changed", "assignment_changed", "escalated"]));
        expect(history.body.filter((entry) => entry.action === "escalated")).toHaveLength(1);
    });
