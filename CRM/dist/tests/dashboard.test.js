...
describe("agent dashboard and work", () => {
    ...
    it("keeps tasks and reminders protected and supports their lifecycle", async () => {
        ...
        expect((await agent.post(`/api/tasks/${task.body.id}/complete`)).body.isCompleted).toBe(true);
        ...
        expect((await agent.post(`/api/reminders/${reminder.body.id}/dismiss`)).status).toBe(200);
    });
    it("adds protected internal comments and immutable ticket history", async () => {
        ...
        expect(comment.body.createdBy).toBe("agent@example.com");
        ...
        expect(history.body.some((entry) => entry.action === "internal_comment_added")).toBe(true);
    });
});
